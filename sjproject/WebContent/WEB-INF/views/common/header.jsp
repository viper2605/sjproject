<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8" import="com.kh.sjproject.member.model.vo.Member"%>
		...
		<div
			...
			<% } else { %>
				...
				<%-- 로그인 영역 --%>
					...
					&nbsp;  <%-- 경로 설정.최상위 경로 요청  (localhost:8080/sjproject 상대 경로 ,(절대 경로도 있음)    --%>
					 <%-- 경로 설정.최상위 경로 요청(컨텍스트페스)(localhost:8080/sjproject 상대 경로 ,(절대 경로도 있음)    --%>
					<a href="<%=request.getContextPath() %>/member/logout.do" role="button" class="btn btn-sm btn-outline-secondary">로그아웃</a>
					
					<%-- 로그인 세션 자체를 만료, 서블릿 만들기  컨트롤러에서 서블릿만들기 --%>
				</div>	
