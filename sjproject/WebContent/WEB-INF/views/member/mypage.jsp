...
<%
	...
	// header.jsp에서 Member 가 import 되어있기 때문에 현재 페이지에서 
	// import는 불필요
	Member member = (Member)request.getAttribute("member");

	// 내 정보에서 전화번호 칸이 3개로 쪼개져서. 배열로 쪼개나눔. '-' 구분자를 이용해 나누기
	
	String[] phone = member.getMemberPhone().split("-");
	// -> 010-111-2222 -> 이것을 split 을 통해 {010,111,2222} 로 쪼개어 나눔
	
	String[] address = {"-", "-", "-"};
	// 주소는 필수 입력 사항 아니므로 - 로 공백 처리, if로 만약 값이 있다면 , 로 주소 쪼개어 나눔
	...
	if(member.getMemberInterest() !=null){
		...
		// for문을 돌려서 interests의 변수 길이만큼 반복하여 그만큼 값 넣어줌
